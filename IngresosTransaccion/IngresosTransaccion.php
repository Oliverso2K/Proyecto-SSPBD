...
<head>
    ...
    <link rel="stylesheet" href="IngresosTransaccionStyle.css">
    <title>Ingresos Individuales por Transacción</title>
</head>
...
<body>
    ...
    <div class="titulo-ventana">
        <h5> Consulta Individual > Ingresos individuales por transacción </h5>
            <h2> INGRESOS INDIVIDUALES POR TRANSACCIÓN </h2>
    </div>
                ...
                <table>
                    ...
                    <?php
                        while ($fila = pg_fetch_array($consultaPersonas)){
                            ...
                            $ingreso = pg_query($conexion,$sql);

                            // while($ingreso_indv = pg_fetch_array($ingreso)){
                        ...
                        ?>
                        <?php while($ingreso_indv = pg_fetch_array($ingreso)){
                            $id_fuente = $ingreso_indv['id_fuente'];
                            $sql = "SELECT * FROM fuentes WHERE id='$id_fuente'";
                            $fuente = pg_query($conexion,$sql);

                            while($fuente_indv = pg_fetch_array($fuente)){
                                $tipo = $fuente_indv['tipo'];
                            }
                        ?>
                        <tr>
                            ...
                            <th>$<?php echo $ingreso_indv['cantidad']?></th>
                        </tr>
                        <?php }?>
